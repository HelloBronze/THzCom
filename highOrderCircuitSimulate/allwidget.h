...
{
public:
    AllWidget(MainWindow *mwindow);//递归的遍历主窗口及子窗口中的所有小部件(?)
    QWidget *addWidget(QString type, QWidget *widBack, QPoint position);//向给定的父窗口添加一个新的小部件
    bool isWidgetInnerOrOuter(QPoint posMouse);//判断鼠标光标位于窗口内部还是外部
